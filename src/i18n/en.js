export default {
    translation: {
      // Auth
      welcome: "Welcome to Lala",
      login: "Login",
      signup: "Create account",
      email: "Email",
      password: "Password",
      fullName: "Full name",
      loginButton: "Sign in",
      signupButton: "Sign up",
      noAccount: "Don't have an account?",
      haveAccount: "Already have an account?",
  
      // Dashboard
      dashboard: "Home",
      safeToSpend: "Safe to spend",
      totalBalance: "Total balance",
      protected: "Protected money",
      upcoming: "Upcoming payments",
      nextPayday: "Next payday",
  
      // Vaults
      vaults: "Vaults",
      addVault: "New vault",
      bills: "Bills",
      debt: "Debt",
      weeklyLiving: "Weekly living",
      travel: "Travel",
      fun: "Fun",
      emergency: "Emergency",
  
      // Transactions
      transactions: "Transactions",
      addTransaction: "New transaction",
      editTransaction: "Edit transaction",
      deleteTransaction: "Delete transaction",
      amount: "Amount",
      category: "Category",
      description: "Description",
      date: "Date",
      essential: "Essential",
      bank: "Bank",
      paymentMethod: "Payment method",
      creditCard: "Credit card",
      selectCard: "Select a credit card",
      noCardsHint: "No credit cards — add one first",
      payInInstallments: "Pay in installments",
      invalidNumCuotas: "Enter at least 2 installments",
  
      // Bills
      billsTitle: "My bills",
      dueDate: "Due date",
      dueDay: "Due day",
      paid: "Paid",
      pending: "Pending",
      addBill: "Add bill",
      editBill: "Edit bill",
      deleteBill: "Delete bill",
      billName: "Bill name",
      billNamePlaceholder: "e.g. Rent, Netflix, Internet",
      billNameRequired: "Bill name is required",
      invalidDueDay: "Enter a valid day (1-31)",
      markAsPaid: "Mark as paid",
      pay: "Pay",
      billsDueThisWeek: "Bills due this week",
      sectionDueThisWeek: "Due this week",
      sectionVaults: "Vaults",
      sectionAllBills: "All bills",
      billsCalendar: "Bills calendar",
      noBills: "No bills yet.",
      addFirstBill: "+ Add first bill",
      deductFrom: "Deduct from",
  
      // Cards
      creditCards: "Credit cards",
      addCard: "Add card",
      cardName: "Card name",
      network: "Network",
      creditLimit: "Credit limit",
      statementDate: "Statement date",
      dueDate: "Due date",
      utilization: "Utilization",
      noCards: "No credit cards yet.",
      addFirstCard: "+ Add first card",
      editCard: "Edit card",
      deleteCard: "Delete card",
      sectionCards: "Cards",
      balance: "Balance",
      limit: "Limit",
  
      // Cuotas
      cuotas: "Installments",
      addCuota: "Cuota",
      payCuota: "Pay installment",
      cuotaDescription: "Description",
      totalAmount: "Total amount",
      numCuotas: "Number of installments",
      cuotaAmount: "Monthly payment",
      startDate: "Start date",
      paidCuotas: "Paid installments",
      remainingCuotas: "Remaining balance",
      noCuotas: "No installments yet.",
      cuotaProgress: "{{paid}} of {{total}}",
      totalCuotas: "Total installments",
      remaining: "Remaining",
  
      // Payday
      payday: "Payday",
      paydayWizard: "Payday wizard",
      itsPayday: "🎉 It's payday!",
      startWizard: "Start",
      step1Income: "Income",
      step2Vaults: "Vaults",
      step3Summary: "Summary",
      paycheck: "Paycheck",
      suggestedFills: "Suggested vault fills for upcoming bills",
      noVaultsLinked: "No bills with linked vaults due in the next 15 days.",
      incomeAdded: "Income added",
      vaultsFilled: "Vaults filled",
      safeToSpendAfter: "Safe to spend",
      setPayday: "Set your payday in Settings",
      payday1: "Pay period 1 (day of month)",
      payday2: "Pay period 2 (day of month)",
      nomina: "Payroll deposit",
      next: "Next",
      confirm: "Confirm",
      salary: "Salary",
      commission: "Commission",
      reimbursement: "Reimbursement",
      allocate: "Allocate",
  
      // Manolo
      pet: "Pet",
      food: "Food",
      health: "Health",
      grooming: "Grooming",
  
      // General
      save: "Save",
      cancel: "Cancel",
      delete: "Delete",
      edit: "Edit",
      add: "Add",
      total: "Total",
      month: "Month",
      week: "Week",
      settings: "Settings",
      language: "Language",
      logout: "Sign out",
      loading: 'Loading...',
      safeToSpendSubtitle: 'You can spend this today',
      noVaults: "You don't have any vaults yet.",
      createFirstVault: '+ Create first vault',
      vaultName: 'Name',
      vaultNamePlaceholder: 'e.g. Rent, Travel, Emergency',
      vaultTarget: 'Goal (COP)',
      vaultCurrent: 'Current amount (COP)',
      editVault: 'Edit vault',
      vaultGoal: 'Goal',
      vaultReady: 'Ready',
      invalidTargetAmount: 'Enter a valid goal amount',

      // Banks
      addBank: 'Add account',
      bankName: 'Bank name',
      bankNamePlaceholder: 'e.g. Bancolombia, Nequi, Davivienda',
      accountType: 'Account type',
      savings: 'Savings',
      checking: 'Checking',
      currentBalance: 'Current balance',
      myAccounts: 'My accounts',
      editBank: 'Edit account',
      deleteAccount: 'Delete account',
      noAccounts: 'No accounts yet.',
      preferences: 'Preferences',
      accountSection: 'Account',

      // Transactions screen
      expense: 'Expense',
      income: 'Income',
      descriptionPlaceholder: 'e.g. Groceries, Salary, Netflix',
      invalidAmount: 'Enter a valid amount',
      selectBank: 'Select an account',
      selectCategory: 'Select a category',
      noBanksHint: 'No accounts — add one first',
      noTransactions: "You don't have any transactions yet.",
      addFirstTransaction: '+ Add first transaction',
      assignVault: 'Assign to vault',
      noVault: 'No vault',
      noVaultsHint: 'No vaults — create one first',
    }
  }